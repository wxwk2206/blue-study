...
```
unserialize 触发 wakeup 或 destruct
→调用 Class A 的魔术方法
 →调用 Class B 的魔术方法
  →调用 Class C 的魔术方法
   →最终执行 eval /system/include
```
### 示例：两跳 POP 链
```php
<?php
...
## 2.8 PHP 反序列化挖掘流程
```
grep 找 unserialize → 找可控参数 → 找 wakeup /destruct → 看是否有危险操作
├─是（有危险操作） → 直接构造 Payload
└─否（无危险操作）→找POP链 →遍历所有魔术方法 →串联到eval system include →构造Payload
```
### grep 命令
```bash
# 找反序列化点
...
```
readObject 触发 → 某个类 A 的 readObject 自定义实现 →调用方法→ 类 B 的某个方法 →调用方法→ 类 C 的某个方法 →最终调用→ Runtime.exec/ ClassLoader
```
---
...
```
 ...
 🎯 **关键**：攻击者不需要会写这些类，**ysoserial 工具会自动生成 payload**。
