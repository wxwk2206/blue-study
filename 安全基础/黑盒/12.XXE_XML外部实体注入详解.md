...
```
XXE漏洞高发场景包含如下类别：
1. Web服务，接收SOAP、REST格式的XML请求报文主体
2. 文档解析，处理Office系列文档，如docx、xlsx
3. 配置文件，解析Spring、pom.xml、web.xml这类配置文件
4. SVG图片，触发域名解析行为
5. RSS以及Atom订阅数据解析
6. SAML单点登录业务
7. SVG头像上传功能
...
<!DOCTYPE book [                         <!-- DTD（文档类型声明，独立部分）
    <!ENTITY author "张三">              <!-- 自定义实体【定义写在DTD】
]>
<book>                                   <!-- 根元素 -->
    <info attr="val">                    <!-- 属性 -->
        &author;                         <!-- 实体引用：出现在元素文本里 -->
        <![CDATA[<html>raw文本]]>        <!-- CDATA -->
    </info>
    <!-- 这是注释 -->                     <!-- 注释 -->
    <?php echo 123 ?>                    <!-- 处理指令 -->
</book>
...
实体 Entity
├─内部实体 定义在 DTD 内
├─外部实体 引用外部资源 → XXE 攻击核心
├─参数实体 仅 DTD 内使用 → Blind XXE 核心
├─通用实体 XML 文档内使用
└─预定义实体 < > & ...
```
### 1. 内部实体（普通通用实体）
...
<note>&author;</note>
```
实体内容直接写在 DTD 里面，就是文本常量。
XML 文档引用：`&author;`，解析器直接替换成字符串`Alice`
...
<note>&xxe;</note>
```
不写死文本，通过`SYSTEM`/`PUBLIC`指向**外部文件、网络 URL**。
...
### 3. 通用实体
- 符号：`&实体名;`
- **可以在 XML 业务文档部分使用**（`<data>&xxx;</data>`）
- 内部、外部实体都可以是通用实体。
